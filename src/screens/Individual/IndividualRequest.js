...
const IndividualRequest = ({navigation}) => {
  const [data1, setData1] = useState(Params)
  const [data, setData] = useState(Params)
  const [btn1, setBtn1] = useState(false)
  const [btn2, setBtn2] = useState(true)

  const AllOne = () => {
    navigation.navigate('individualrrtabbar')
  }
  const CreditTwo = () => {
    setBtn2(true)
  }
          ...
          <View>
            ...
            {data1.map((val) => {
              return (
                <>
                  <TouchableOpacity
                    onPress={() => navigation.navigate('individualreqdetails')}>
